...
runs="${3:-3}"
max_tokens_list="${4:-128,512,2048}"
warmup="${5:-1}"
...
case "$size" in
  ...
  4b)
    ...
    ;;
  *)
    echo "Usage: $0 [2b|4b] [base_url] [runs] [max_tokens_csv] [warmup]" >&2
    exit 2
    ;;
esac
...
.venv/bin/python backend_compare/benchmarks/bench_streaming_backend.py \
  --model "$model" \
  --base-url "$base_url" \
  ...
  --out-dir backend_compare/results/rtx4070/stream_requests
